...
**Magasin physique** la collecte de listes peut présenter des défis en raison de la saisie verbale ou écrite de l’adresse, ce qui entraîne une faute d’orthographe dans les adresses. Il est recommandé d’envoyer un email de confirmation le plus rapidement possible après les inscriptions en magasin.

La forme la plus courante de **abonnement à un site web** est &quot;inclusion unique&quot;. Il s’agit de la norme minimale absolue que vous devez utiliser pour acquérir des adresses électroniques. L’option d’opt-in unique est lorsque le détenteur d’une adresse email spécifique accorde à l’expéditeur l’autorisation d’envoyer des emails marketing, généralement en envoyant l’adresse via un formulaire web ou des inscriptions en magasin. Bien qu’il soit possible d’exécuter une campagne par e-mail réussie à l’aide de cette méthode, cela peut être la cause de certains problèmes.
...
Pour obtenir des conseils sur la manière de minimiser les problèmes présentés à la fois dans le magasin physique et dans l’inclusion unique, accédez à la page [Qualité et hygiène des données](#data-quality-and-hygiene) pour connaître les détails et les avantages du double opt-in.
